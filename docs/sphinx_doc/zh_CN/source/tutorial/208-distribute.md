...
上面介绍的案例都是将一个已经初始化的 Agent 通过 `to_dist` 方法转化为其分布式版本，相当于要执行两次初始化操作。如果直接使用 `to_dist` 方法会严重影响运行效率。为此 AgentScope 也提供了在初始化 Agent 实例的同时将其转化为其分布式版本的方法，即在原 Agent 实例初始化时传入 `to_dist=True` 以及其他必要参数。
