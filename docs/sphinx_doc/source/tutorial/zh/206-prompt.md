...
**提示（prompt）**是与语言模型互动时的关键组件，尤其是当寻求生成特定类型的输出或指导模型朝向期望行为时。本教程将指导您如何使用 `PromptEngine` 类，它简化了为大语言模型（LLMs）制作提示的过程。
...
- **缩减原则**：提供了两种处理超出最大长度提示的方案：`ShrinkPolicy.TRUNCATE` 简单地截断提示，以及 `ShrinkPolicy.SUMMARIZE `总结部分对话历史以节省空间。
