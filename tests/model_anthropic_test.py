...
class TestAnthropicChatModel(IsolatedAsyncioTestCase):
    ...
    async def test_call_with_structured_model_warning(self) -> None:
        ...
        with patch("anthropic.AsyncAnthropic") as mock_client_class:
            ...
            with patch(
                ...
                self.assertIn(
                    "structured_model is provided",
                    warning_message,
                )
                self.assertIn("tools", warning_message)
                self.assertIn("tool_choice", warning_message)
    ...
    async def test_structured_model_without_tools_in_response(
        self,
    ) -> None:
    ...
    async def test_structured_model_with_invalid_json_input(self) -> None:
        ...
        with patch("anthropic.AsyncAnthropic") as mock_client_class:
            ...
            final_response = responses[-1]
            self.assertEqual(len(final_response.content), 1)
            expected_content = [
                ToolUseBlock(
                    type="tool_use",
                    id="invalid_123",
    ...
    async def test_structured_model_tool_choice_formatting(self) -> None:
        ...
        with patch("anthropic.AsyncAnthropic") as mock_client_class:
            ...
            self.assertEqual(
                call_args["tools"][0]["name"],
                "format_output",
            )

    class TestAnthropicIntegrationScenarios(IsolatedAsyncioTestCase):
        """Integration test scenarios for Anthropic model."""

        async def test_complete_conversation_with_system_message(
            self,
        ) -> None:
        ...
        async def test_multi_turn_conversation_with_complex_tools(
            self,
        ) -> None:
            ...
            with patch("anthropic.AsyncAnthropic") as mock_client_class:
                ...
                messages = [
                    {
                        "role": "user",
                        "content": "Analyze the weather and recommend "
                        "activities",
                    },
                ]
                tools = [
                    ...
                    },
                    {
                        ...
                        "function": {
                            ...
                            "description": "Suggest activities based on "
                            "weather",
                            "parameters": {
                                "type": "object",
                                "properties": {
