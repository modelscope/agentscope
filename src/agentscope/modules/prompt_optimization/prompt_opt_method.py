...
def find_top_k_embeddings(query_embedding, list_embeddings, k):
    ...
    return [
        (index, embedding, score)
        for index, embedding, score in zip(
            top_k_indices,
            top_k_embeddings,
            top_k_scores,
        )
...
class ExamplePromptOptMethod(PromptOptMethodBase):
    ...
    def optimize(self, user_prompt: str) -> str:
        ...
        logger.chat(f"Optimizing user prompt, original prompt{user_prompt} \n")
        response = self.model(prompt).text
        logger.chat(f"Optimized prompt: {response} \n")
