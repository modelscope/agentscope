...
function initializeDashboardDetailDialoguePage(pRuntimeInfo) {
  ...
  inputTextArea.addEventListener("keydown", function (e) {
    if (e.key === "Enter" && !e.ctrlKey && !e.metaKey) {
      ...
      const textBeforeCursor = inputTextArea.value.substring(0, cursorPosition);
      const textAfterCursor = inputTextArea.value.substring(cursorPosition);
    ...
    .then(() => {
      ...
      fetch(
        ...
        .then((data) => {
          ...
          const chatRows = data.map((msg, index) =>
          ...
          socket.on("connect", () => {
            ...
            sendBtn.onclick = () => {
              ...
              const url = _obtainAllUrlFromFileList();
          ...
          socket.on("display_message", (data) => {
            if (data.run_id === pRuntimeInfo.run_id) {
              ...
              for (let index = chatRows.length - 1; index >= 0; index--) {
                ...
                const rowDataMsg = JSON.parse(row.getAttribute("data-msg"));
